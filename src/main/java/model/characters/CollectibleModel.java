...
public class CollectibleModel extends GeoShapeModel {
    ...
    public CollectibleModel(GeoShapeModel geoShapeModel) {
        ...
        this.setAnchorSave(
                new Point2D.Float((COLLECTIBLE_SIZE_OFFSET.getValue() + geoShapeModel.getCollectibleValue()) / 2f,
                        (COLLECTIBLE_SIZE_OFFSET.getValue() + geoShapeModel.getCollectibleValue()) / 2f)
        );

        createCollectible(getModelId(), geoShapeModel.getModelId(), geoShapeModel.getCollectibleValue(),
                roundPoint(getAnchorSave()), geoShapeModel.getMotionPanelId());

        Point2D spawnLocation = addUpPoints(geoShapeModel.getAnchor(),
