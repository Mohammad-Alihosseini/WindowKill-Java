...
public enum MovementConstants {
    ...
    MAX_SAFE_ROTATION, DEFAULT_SPEED, BULLET_SPEED, DEFAULT_DECELERATION, MOTION_PANEL_SPEED,
    DECELERATION_DECAY, DECELERATION_SENSITIVITY, DAMPEN_FACTOR, POSITION_UPDATE_INTERVAL;

    public float getValue() {
        return switch (this) {
            case EPSILON_SPEED ->
                    Profile.getCurrent().getSizeScale() * Profile.getCurrent().getGameSpeed() * 80f / Profile.getCurrent().getUps();
            case ANGULAR_SPEED_BOUND -> 400f / Profile.getCurrent().getUps();
            case DEFAULT_ANGULAR_DECAY ->
                    1 - (0.5f * Profile.getCurrent().getSizeScale() * Profile.getCurrent().getGameSpeed() / Profile.getCurrent().getUps());
            case MAX_SAFE_ROTATION -> 240f / Profile.getCurrent().getUps();
            case DEFAULT_SPEED ->
                    Profile.getCurrent().getSizeScale() * Profile.getCurrent().getGameSpeed() * 30f / Profile.getCurrent().getUps();
            case BULLET_SPEED ->
            ...
            case MOTION_PANEL_SPEED -> (int) (2400 / Profile.getCurrent().getGameSpeed());
            ...
            case POSITION_UPDATE_INTERVAL ->
                    TimeUnit.SECONDS.toNanos(1) / (Profile.getCurrent().getGameSpeed() * Profile.getCurrent().getUps());
        };
    }
}
